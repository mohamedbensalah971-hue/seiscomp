...
class SettingsScreen extends StatelessWidget {
  ...
  void _showResetConfirmation(BuildContext context, String lang, AppState appState) {
    showDialog(
      context: context,
        ...
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              child: const Text('Confirm Reset', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
            ),
          ],
        );
      },
        ...
        child: Directionality(
          ...
          child: ListView(
            ...
            children: [
              ...
              _buildSectionHeader(context, AppLocalizations.get('language_sec', lang)),
              const SizedBox(height: 8),
              _buildLanguageCard(appState),
              const SizedBox(height: 24),

              // Audio controller card
              _buildSectionHeader(context, AppLocalizations.get('sound_sec', lang)),
              const SizedBox(height: 8),
              _buildAudioCard(context, appState, lang),
              const SizedBox(height: 24),

              // Accessibility config card
              _buildSectionHeader(context, AppLocalizations.get('accessibility_sec', lang)),
              const SizedBox(height: 8),
              _buildAccessibilityCard(context, appState, lang),
              const SizedBox(height: 24),

              // Reset / Storage card
              _buildSectionHeader(context, AppLocalizations.get('data_sec', lang)),
              const SizedBox(height: 8),
  ...
  Widget _buildSectionHeader(BuildContext context, String title) {
    return Text(
      title,
      style: AppTextStyles.displaySmall(context).copyWith(color: AppColors.secondary),
    );
  }
  ...
  Widget _buildAccessibilityCard(BuildContext context, AppState appState, String lang) {
    return GlassmorphicContainer(
  ...
  Widget _buildDataPrivacyCard(BuildContext context, AppState appState, String lang) {
    return GlassmorphicContainer(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
          ...
          if (appState.selectedProfile != null)
            ListTile(
              title: const Text('Delete Current Profile', style: TextStyle(color: AppColors.accentRed)),
              leading: const Icon(Icons.person_remove_outlined, color: AppColors.accentRed),
              onTap: () async {
